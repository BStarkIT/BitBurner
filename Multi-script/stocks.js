...
export async function main(ns) {
  ...
  const moneyKeep = 100000; // Failsafe Money
  ...
  const toastDuration = 1500;   // Toast message duration
  ...
  function format(number) {
    ...
    const answer = ns.formatNumber(number, 3, 1000);;
    if (answer === "NaN") {
      return `${number}`;
    }
    return answer;
  ...
  function formatReallyBigNumber(number) {
    ...
    // Use formatNumber for numbers it can format
  ...
  function sellIfOutsideThreshdold(stock) {
    ...
    if (position[0] > 0) {
      ...
      ns.print('      Position -> ' + ns.formatNumber(position[0], 3, 1000));
      ns.print('      Profit -> ' + ns.formatNumber(profit, 3, 1000));
      // Check if we need to sell Long stocks
      if (forecast < sellThreshold_Long) {
        let soldFor = ns.stock.sellStock(stock, position[0]);
        let cash = position[0] * soldFor
        let message = 'Sold ' + position[0] + ' Long shares of ' + stock + ' for ' + ns.formatNumber(soldFor, 3, 1000) + ' Cash ' + cash;
        ns.toast(message, 'success', toastDuration);
        ns.tprint(message)
      }
    }
    if (shortUnlock) {
      if (position[2] > 0) {
        ...
        if (forecast > sellThreshold_Short) {
          ...
          let message = 'Sold ' + stock + ' Short shares of ' + stock + ' for ' + ns.formatNumber(soldFor, 3, 1000);
          ns.toast(message, 'success', toastDuration);
        }
      }
    }
