...
export async function main(ns) {
	...
	servers.forEach(
		(server) => {
			var file = ns.ls(server, 'cct')
			if (file == 0) {
				ns.print("nothing found on " + server)
			}
			else {
				const contract = file[0]
				const type = ns.codingcontract.getContractType(contract, server);
				const info = ns.codingcontract.getData(contract, server);
				output.push({ server: server, file: contract, type: type, data: info });
			}
		}
	)
	if (output.length == 0) {
		...
		let found = 0;
		for (const contract of output) {
			found++;
			let didSolve = false;
			try { didSolve = await solve(contract.type, contract.data, contract.server, contract.file, ns); } catch { }
			if (didSolve) {
				ns.toast(`INFO: Solved ` + contract.type + ' (' + didSolve + ')');
				ns.print(ns, `INFO: Solved ` + contract.type + ' (' + didSolve + ')')
				ns.rm('Contracts.txt', 'home')
			}
			else {
				ns.toast(`FAIL: ${contract.server} - ${contract.file} - ${contract.type} - ${didSolve || "FAILED!"}`);
				ns.print(ns, `FAIL: ${contract.server} - ${contract.file} - ${contract.type} - ${didSolve || "FAILED!"}`)
				ns.rm('Contracts.txt', 'home')
			}
		...
		if (found > 0)
			ns.toast(`Found ${found} contracts`);
	}
}
async function solve(type, data, server, contract, ns) {
	...
	if (solution == 'none') {
		ns.print('ERROR: NO SOLVER FOR ' + contract + ' ' + solution);
		return '';
	}
